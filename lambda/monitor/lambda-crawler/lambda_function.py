...
def lambda_handler(event, context):
    try:
        ...
        ### S3 업로드 ###
        object_name = f"monitor/community/{community}/{start_datetime.strftime('%Y%m%d')}/{start_datetime.strftime('%Y%m%d_%H%M')}_{end_datetime.strftime('%Y%m%d_%H%M')}_{community}_{query}.csv"
